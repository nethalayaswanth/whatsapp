...
const DrawerHeader = ({ name,className,style,disabled, onClick }) => {
  ...
  return (
    <header
      style={style}
      className={`flex flex-col justify-end h-[108px] pr-[20px] pl-[23px] flex-none bg-panel-header-coloured ${className}`}
    >
      <div className="flex items-center w-[full] p-0 bg-inherit h-[59px] text-white">
        <div className="w-[53px] flex items-center justify-start">
          <button
            disabled={disabled}
            className="m-0 p-0 outline-none border-0 cursor-pointer text-white disabled:text-opacity-[0.8] "
            onClick={handleClick}
          >
            <Arrow />
          </button>
